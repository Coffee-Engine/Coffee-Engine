(function () {
   ...
   */
  const blockEvents = new Set([Blockly.Events.VIEWPORT_CHANGE, Blockly.Events.BLOCK_CHANGE, Blockly.Events.BLOCK_CREATE, Blockly.Events.BLOCK_DELETE, Blockly.Events.BLOCK_DRAG, Blockly.Events.BLOCK_MOVE]);
  ...
  class FocusRegion {
     ...
     */
    constructor(primaryWorkspace, minimapWorkspace) {
    ...
    init() {
      ...
      this.svgGroup = Blockly.utils.dom.createSvgElement(Blockly.utils.Svg.G, { class: "blockly-focus-region" }, null);
      // Make the mask under the svg group.
      const mask = Blockly.utils.dom.createSvgElement(new Blockly.utils.Svg("mask"), { id: "focusRegionMask" + this.id }, this.svgGroup);
    ...
    update() {
      ...
      const scale = minimapContent.width / minimapMetrics.getContentMetrics(true).width;
...
`);
  ...
  sugarcube.FocusRegion = FocusRegion;
})();

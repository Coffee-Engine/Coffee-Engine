  ...
  coffeeEngine.classes.node = class {
    ...
    set parent(value) {
      if (value.addChild) {
        ...
        if (
          !coffeeEngine.runtime.currentScene.hasEventListener("update", () => {
            this.update();
          })
        ) {
          coffeeEngine.runtime.currentScene.addEventListener("update", () => {
            this.update();
          });
        }
        if (
          !coffeeEngine.runtime.currentScene.hasEventListener("draw", () => {
            this.draw();
          })
        ) {
          coffeeEngine.runtime.currentScene.addEventListener("draw", () => {
            this.draw();
          });
        }
